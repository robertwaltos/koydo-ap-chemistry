...
import { EXAM_CONFIG } from "@/lib/ap-chemistry/config";

describe("AP Chemistry Config", () => {
  it("has correct exam ID", () => {
    expect(EXAM_CONFIG.examId).toBe("EXAM174");
  });

  it("has correct slug", () => {
    expect(EXAM_CONFIG.slug).toBe("ap-chemistry");
  });
